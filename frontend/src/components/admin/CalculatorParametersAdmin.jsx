...
const CalculatorParametersAdmin = () => {
  ...
  const seedFromMockIfEmpty = async () => {
    try {
      ...
      const resp = await fetch(`${API_CONFIG.BACKEND_URL}/api/admin/migrate-frontend-data`, {
        method: 'POST',
        headers: {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (Array.isArray(data) && data.length === 0) {
          // Auto-seed from mock migration when empty
          await seedFromMockIfEmpty();
          const reResp = await fetch(`${API_CONFIG.BACKEND_URL}/api/admin/calculator-parameters`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
          if (reResp.ok) {
            const seeded = await reResp.json();
