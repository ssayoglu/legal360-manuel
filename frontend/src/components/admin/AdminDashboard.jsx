...
import { legalProcesses, mockBlogPosts, mockSupremeCourtDecisions } from '../../data/mock';
...
const AdminDashboard = ({ adminUser }) => {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      // Mock data fallback
      setStats({
        total_processes: legalProcesses.length,
        total_blog_posts: mockBlogPosts.length,
        total_decisions: mockSupremeCourtDecisions.length,
        total_visits: 1250,
  ...
  const getFilteredData = () => {
    ...
    switch (dataType) {
      ...
      case 'blog':
        ...
        break;
      case 'decisions':
        data = mockSupremeCourtDecisions;
        break;
      default:
        data = [];
    }
  ...
  const getCategories = () => {
    ...
    } else if (dataType === 'decisions') {
      mockSupremeCourtDecisions.forEach(item => {
        if (item.category) categories.add(item.category);
      });
    }
