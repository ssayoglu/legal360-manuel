...
const ProcessFlowWithHeader = () => {
  ...
  const renderCalculator = () => {
    ...
    if ((currentProcess.calculator_type || currentProcess.calculatorType) === 'compensation') {
      return <CompensationCalculator onClose={() => setShowCalculator(false)} />;
    } else if ((currentProcess.calculator_type || currentProcess.calculatorType) === 'execution' || (currentProcess.calculator_type || currentProcess.calculatorType) === 'sentence') {
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {(selectedStep.required_documents || []).map((doc, index) => (
                          <div key={index} className="flex items-start p-3 bg-gray-50 rounded-lg group relative">
                            <div className="w-2 h-2 bg-blue-500 rounded-full mr-3 mt-2 flex-shrink-0"></div>
                            <span className="text-gray-700 leading-relaxed cursor-help">
                              {typeof doc === "string" ? doc : doc.name || "Belge adı belirtilmemiş"}
                            </span>
                            {typeof doc === "object" && doc.description && (
                              <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-800 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-10 max-w-xs">
                                {doc.description}
                              </div>
                            )}
                          </div>
